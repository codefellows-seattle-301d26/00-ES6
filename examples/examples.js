...
let classrooms = ['Lovelace', 'Babbage', 'Hopper', 'Turing'];

// OLD WAY: Concatenation
let oldWay = 'Code 101 meets in ' + classrooms[0] + ', Code 201 meets in ' + classrooms[2] + ', Code 301 meets in ' + classrooms[1] + ', and Code 401 meets in ' + classrooms[3] + '.';

// NEW WAY: Template literal notation
let newWay = `Code 101 meets in ${classrooms[0]}, Code 201 meets in ${classrooms[2]}, Code 301 meets in ${classrooms[1]}, and Code 401 meets in ${classrooms[3]}.`;
...
let mmm = ['chocolate', 'vanilla', 'strawberry'];
let iceCreamCone = document.getElementById('cone');

function scoop(){
  let good = Math.floor(Math.random() * mmm.length);

  // let scoop = `<img src="img/${mmm[good]}.jpg" title="${mmm[good]}" />`;
  let scoop = '<img src="img/' + mmm[good] + '.jpg" title="' + mmm[good] + '" />';
